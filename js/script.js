 ...
 * CORRECCIÓN: Lógica de cálculo de postes ajustada a nueva especificación.
 * ACTUALIZACIÓN: Lógica de cálculo de fijaciones (Clavos, Fulminantes, Tornillos 1", Tornillos 1/2") implementada según nueva especificación centralizada y con redondeo/merma global.
 * CORRECCIÓN: Lógica de cálculo de Tornillos de 1" en Muros corregida para manejar paneles Exteriores e Interiores en un mismo muro.
 * CORRECCIÓN: Ajustes de sintaxis, corrección de llave faltante en performSingleItemCalculations, y re-verificación de re-declaración de variables.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // NOTA: Ya no se usa para redondear cantidades por ítem antes de sumar.
    ...
    // NOTA: Fulminantes NO se inicializa aquí, su cálculo es GLOBAL.
    const getFinishingMaterials = (panelType) => {
         ...
         // Asegúrate de que Fulminantes NO se inicialice aquí. Su cálculo es global.
         return finishing;
    };
    ...
    const updateSegmentItemSummary = (segmentBlock) => {
        ...
        if (type === 'muro') {
            ...
            const cieloPanelType = cieloPanelSelect && !cieloPanelSelect.closest('.hidden') ? cieloPanelTypeSelect.value : 'N/A';
    ...
    const createItemBlock = () => {
     ...
     // Esta función ahora SOLO lee inputs y actualiza la visualización del metraje del ítem y sus segmentos.
     // La lógica de cálculo de metraje por ítem se moverá a la función de cálculo principal por ítem.
     const calculateAndDisplayItemMetraje = (itemBlock) => {
         ...
         // Limpia el contenido anterior
          ...
          // Oculta metrajes de segmentos por defecto y los limpia
          itemBlock.querySelectorAll('.segment-metraje-display').forEach(div => {
              div.textContent = '';
              div.style.color = 'inherit';
              // div.classList.add('hidden'); // Mantener visible pero vacío si no aplica/error
          });
         ...
         try {
                 ...
                 segmentBlocks.forEach((segBlock, index) => {
                     ...
                     const segmentNumber = index + 1;
                               ...
                               if (segmentMetrajeDisplayDiv) {
                                   ...
                                   // segmentMetrajeDisplayDiv.classList.remove('hidden'); // Asegurar que esté visible
                               }
                           });
             ...
             // Oculta el div de metraje del segmento si el tipo no es muro o cielo.
              if (type !== 'muro' && type !== 'cielo') {
                   itemBlock.querySelectorAll('.segment-metraje-display').forEach(div => div.textContent = ''); // Limpia el texto
              }
     ...
     // Devuelve un objeto con las cantidades FLOAT de materiales (incluyendo fijaciones FLOAT) y el metraje del ítem.
     const performSingleItemCalculations = (itemSpecs) => {
         const { id, number, type } = itemSpecs;
         let itemOtherMaterialsFloat = {};
         let itemPanelDetails = { // Details for updating global panel accumulators
             panelTypeFace1: null,
             panelTypeFace2: null,
             cieloPanelType: null,
             cenefaPanelType: null,
             muroSegmentsArea: [], // Array of { segmentArea, panelType } for muro segments
             cieloSegmentsArea: [] // Array of { segmentArea, panelType } for cielo segments
         };
         let itemMetraje = { area: 0, linear: 0 }; // Store calculated metraje for this item
         ...
         // Need to pass the panel type correctly based on item type.
         ...
         itemOtherMaterialsFloat = getFinishingMaterials(initialFinishingPanelType);
         // Asegurarse de que las claves de tornillos y clavos existan si no fueron inicializadas por getFinishingMaterials, para poder sumarles FLOAT
         if (itemOtherMaterialsFloat['Tornillos de 1" punta fina'] === undefined) itemOtherMaterialsFloat['Tornillos de 1" punta fina'] = 0;
         if (itemOtherMaterialsFloat['Tornillos de 1" punta broca'] === undefined) itemOtherMaterialsFloat['Tornillos de 1" punta broca'] = 0;
         if (itemOtherMaterialsFloat['Tornillos de 1/2" punta fina'] === undefined) itemOtherMaterialsFloat['Tornillos de 1/2" punta fina'] = 0;
         if (itemOtherMaterialsFloat['Tornillos de 1/2" punta broca'] === undefined) itemOtherMaterialsFloat['Tornillos de 1/2" punta broca'] = 0;
         if (itemOtherMaterialsFloat['Clavos con Roldana'] === undefined) itemOtherMaterialsFloat['Clavos con Roldana'] = 0;
         // Fulminantes NO se inicializa aquí, su cálculo es global.


         if (type === 'muro') {
              const { faces, postSpacing, postType, isDoubleStructure, cara1PanelType, cara2PanelType, segments, totalMuroArea, totalMuroWidth, metrajeArea } = itemSpecs;

              itemMetraje.area = metrajeArea; // Use the calculated metrajeArea from itemSpecs

              // Store panel details for accumulator update
              itemPanelDetails.panelTypeFace1 = cara1PanelType;
              itemPanelDetails.panelTypeFace2 = cara2PanelType;
              itemPanelDetails.muroSegmentsArea = segments.filter(seg => seg.isValidForMaterials).map(seg => ({
                   segmentArea: seg.area,
                   panelTypeFace1: cara1PanelType,
              ...
              const totalMuroWidthForStructureCalc = totalMuroWidth; // Use the sum of valid widths
              const totalMuroAreaForPanelsFinishingCalc = totalMuroArea; // Use the sum of valid areas

              // Postes (based on total valid width and SELECTED POSTE TYPE) - New logic here
              ...
              // Constants used in this specific calculation logic
              const largo_max_poste = 3.66;
              const longitud_empalme = 0.30;


              // Lógica del Cálculo Horizontal de Postes (Num_Horizontal)
              let Num_Horizontal;
              // Usamos ancho_muro y postSpacing directamente de las variables desestructuradas/derivadas
              if (ancho_muro <= 0) {
                  Num_Horizontal = 0;
              } else if (ancho_muro > 0 && ancho_muro < postSpacing) { // Usar postSpacing (desestructurado)
                  Num_Horizontal = 2;
              } else { // ancho_muro >= postSpacing
                  Num_Horizontal = Math.floor(ancho_muro / postSpacing) + 1; // Usar postSpacing (desestructurado)
              }
               // console.log(`Muro #${itemSpecs.number}: Num_Horizontal = ${Num_Horizontal} (ancho=${ancho_muro}, esp=${postSpacing})`); // Log para depuración


              // Cálculo del Número Total de Postes para Estructura Simple (Total_Postes_Simple)
              let Total_Postes_Simple;
              if (alto_muro <= 0 || Num_Horizontal === 0) { // Usar Num_Horizontal calculado
                  Total_Postes_Simple = 0;
              } else if (alto_muro > 0 && alto_muro <= largo_max_poste) {
                  Total_Postes_Simple = Num_Horizontal;
              } else { // alto_muro > largo_max_poste
                  let alto_con_empalme = alto_muro + longitud_empalme;
                  // Usando la fórmula del paso 2/3 proporcionada por el usuario para cantidad_paso2:
                  // ((ancho_muro / espaciamiento_poste) + 1) * alto_con_empalme
                  // Usar ancho_muro y postSpacing (desestructurado)
                  let cantidad_paso2 = ((ancho_muro / postSpacing) + 1) * alto_con_empalme;
                  // Nota: La fórmula para cantidad_paso2 usa (ancho_muro / espaciamiento_poste) + 1
                  // que puede ser ligeramente diferente del Num_Horizontal calculado previamente (por el floor o el caso < espaciamiento).
                  // Implementamos EXACTAMENTE como se describió para cantidad_paso2.
                  Total_Postes_Simple = Math.ceil(cantidad_paso2 / largo_max_poste);
              }
               // console.log(`Muro #${itemSpecs.number}: Total_Postes_Simple = ${Total_Postes_Simple} (alto=${alto_muro})`); // Log para depuración


              // Cálculo del Número Total de Postes Final (Total_Postes_Final)
              let Total_Postes_Final;
              if (isDoubleStructure) { // Usar isDoubleStructure (desestructurado)
                  Total_Postes_Final = Total_Postes_Simple * 2;
              } else {
                  Total_Postes_Final = Total_Postes_Simple;
              }
               // console.log(`Muro #${itemSpecs.number}: Total_Postes_Final = ${Total_Postes_Final} (estructura doble=${isDoubleStructure})`); // Log para depuración


              // Asigna la cantidad final calculada al material específico (Poste por su tipo)
              if (postType) { // Usar postType (desestructurado)
                  itemOtherMaterialsFloat[postType] = (itemOtherMaterialsFloat[postType] || 0) + Total_Postes_Final; // Sumar la cantidad float (que aquí es entera)
                  // Elimina la clave genérica 'Postes' si existía de la lógica inicial de materiales de acabado
                  delete itemOtherMaterialsFloat['Postes']; // Asegura que no quede un poste genérico
              }
              // --- End New Post Calculation Logic ---


              // Canales (based on total valid width) - Keep existing logic, sum FLOAT
              let canalesFloat;
              ...
              // Add to the float total (initialized earlier by getFinishingMaterials if applicable)
              itemOtherMaterialsFloat[cara1PanelType === 'Exterior' ? 'Canales Calibre 20' : 'Canales'] = (itemOtherMaterialsFloat[cara1PanelType === 'Exterior' ? 'Canales Calibre 20' : 'Canales'] || 0) + canalesFloat;


              // Acabado (Pasta, Cinta, Lija / Basecoat, Cinta Malla) - Keep existing logic, sum FLOAT
              ...
              if (primaryPanelTypeForFinishing === 'Normal' || primaryPanelTypeForFinishing === 'Resistente a la Humedad' || primaryPanelTypeForFinishing === 'Resistente al Fuego' || primaryPanelTypeForFinishing === 'Alta Resistencia') {
                  ...
                  itemOtherMaterialsFloat['Pasta'] = (itemOtherMaterialsFloat['Pasta'] || 0) + (totalMuroAreaForPanelsFinishingCalc > 0 ? totalMuroAreaForPanelsFinishingCalc / 22 : 0); // Area / Rendimiento (22 m2/caja)

                  // Cinta de Papel - Calculation based on Total Accumulated Area
                  itemOtherMaterialsFloat['Cinta de Papel'] = (itemOtherMaterialsFloat['Cinta de Papel'] || 0) + (totalMuroAreaForPanelsFinishingCalc > 0 ? totalMuroAreaForPanelsFinishingCalc * 1 : 0); // 1 meter per m2

                  // Lija Grano 120 - Calculation based on Total Accumulated Area
                  itemOtherMaterialsFloat['Lija Grano 120'] = (itemOtherMaterialsFloat['Lija Grano 120'] || 0) + (totalMuroAreaForPanelsFinishingCalc > 0 ? (totalMuroAreaForPanelsFinishingCalc / PANEL_RENDIMIENTO_M2) / 2 : 0);
                  ...
                  itemOtherMaterialsFloat['Basecoat'] = (itemOtherMaterialsFloat['Basecoat'] || 0) + (totalMuroAreaForPanelsFinishingCalc > 0 ? totalMuroAreaForPanelsFinishingCalc / 8 : 0); // Area / Rendimiento (8 m2/saco)

                  // Cinta malla (Durock) - Calculation based on Total Accumulated Area
                  itemOtherMaterialsFloat['Cinta malla'] = (itemOtherMaterialsFloat['Cinta malla'] || 0) + (totalMuroAreaForPanelsFinishingCalc > 0 ? totalMuroAreaForPanelsFinishingCalc * 1 : 0); // 1 meter per m2
              }


              // --- Start New Fastener Calculation Logic (Muro Item) - Uses FLOAT quantities ---

              // 1. Clavos con Roldana (Uso en Muros)
              // Formula: Cantidad Total de Canales (Muros) x 8
              // Usamos la cantidad float de canales calculada para este item (totalCanalesFloatThisItem) y multiplicamos por 8.
              // La cantidad total de canales Float ya está sumada en la clave correspondiente de itemOtherMaterialsFloat.
              const totalCanalesFloatThisItem = (itemOtherMaterialsFloat['Canales'] || 0) + (itemOtherMaterialsFloat['Canales Calibre 20'] || 0);
              itemOtherMaterialsFloat['Clavos con Roldana'] = (itemOtherMaterialsFloat['Clavos con Roldana'] || 0) + (totalCanalesFloatThisItem * 8);


              // 3. Tornillos de 1" (Uso en Muros - Fijación de Paneles)
              // Lógica: Calcular tornillos por cada cara, sumando si el muro tiene 2 caras con distintos paneles.
              // Basado en Cantidad estimada de Paneles por Cara (Float) x 40.
              // totalMuroAreaForPanelsFinishingCalc y faces ya están disponibles.

              if (totalMuroAreaForPanelsFinishingCalc > 0 && faces > 0) {
                  // Calcula el área cubierta por una sola cara
                  const areaPerFace = totalMuroAreaForPanelsFinishingCalc / faces;
                  // Estima la cantidad FLOAT de paneles necesaria para UNA cara
                  const estimatedItemPanelsPerFaceFloat = areaPerFace / PANEL_RENDIMIENTO_M2;

                  // Calcula los tornillos para la Cara 1
                  if (cara1PanelType === 'Exterior') {
                      // Si la Cara 1 usa panel Exterior, agrega tornillos Punta Broca
                      itemOtherMaterialsFloat['Tornillos de 1" punta broca'] = (itemOtherMaterialsFloat['Tornillos de 1" punta broca'] || 0) + (estimatedItemPanelsPerFaceFloat * 40);
                      ...
                      itemOtherMaterialsFloat['Tornillos de 1" punta fina'] = (itemOtherMaterialsFloat['Tornillos de 1" punta fina'] || 0) + (estimatedItemPanelsPerFaceFloat * 40);
                  }

                  // Calcula los tornillos para la Cara 2 (solo si el muro tiene 2 caras y se especificó un tipo de panel)
                  // cara2PanelType ya está disponible.
              ...
              // Formula adaptada: Cantidad FLOAT de Postes (Item) x 4
              // Usamos la cantidad FLOAT de Postes calculada con la nueva lógica de postes para este item (Total_Postes_Final).
              // Decisión de Punta basada en el tipo de panel de la Cara 1 (consistente con lógica previa y simple adaptación)
              // Nota: La nueva lógica de postes calcula Total_Postes_Final como un entero (ceil).
              // Si la lógica de fijaciones requiere float aquí, deberíamos usar el valor float ANTES del ceil de Total_Postes_Simple.
              // Sin embargo, la fórmula indica "Cantidad de Postes... x 4", que sugiere usar la cantidad de postes contados.
              // Mantengamos usar Total_Postes_Final (que ya es redondeado por la lógica de postes) para esta multiplicación.
              if (Total_Postes_Final > 0) { // Usamos la variable calculada con la nueva lógica de postes
                 const postesQuantityForFasteners = Total_Postes_Final; // Usa el número entero de postes

                  if (cara1PanelType === 'Exterior') { // Usamos cara1PanelType para decidir la punta
                      // Si Cara 1 es Exterior, usar Punta Broca para estas conexiones estructurales
                      itemOtherMaterialsFloat['Tornillos de 1/2" punta broca'] = (itemOtherMaterialsFloat['Tornillos de 1/2" punta broca'] || 0) + (postesQuantityForFasteners * 4);
                  } else { // Si Cara 1 es Interior
                      // Si Cara 1 es Interior, usar Punta Fina para estas conexiones estructurales
                       itemOtherMaterialsFloat['Tornillos de 1/2" punta fina'] = (itemOtherMaterialsFloat['Tornillos de 1/2" punta fina'] || 0) + (postesQuantityForFasteners * 4);
                  }
              }

              // Fulminantes (Cálculo GLOBAL, no por ítem. Inicializado a 0 si aplica) - Lógica movida al final de calculateMaterials


              // --- End New Fastener Calculation Logic (Muro Item) ---


         } else if (type === 'cielo') {
              const { plenum, angularDeduction, cieloPanelType, segments, totalCieloArea, totalCieloPerimeterSum, metrajeArea } = itemSpecs;

              itemMetraje.area = metrajeArea; // Use the calculated metrajeArea from itemSpecs

              // Store panel details for accumulator update
              itemPanelDetails.cieloPanelType = cieloPanelType;
              itemPanelDetails.cieloSegmentsArea = segments.filter(seg => seg.isValidForMaterials).map(seg => ({
                  segmentArea: seg.area,
              ...
              const totalCieloAreaForPanelsFinishingCalc = totalCieloArea; // Use sum of valid areas
              const totalCieloPerimeterForAngularCalc = totalCeloPerimeterSum; // Use sum of valid perimeters

               // Canal Listón (based on total valid area) - Keep existing logic, sum FLOAT
               ...
               itemOtherMaterialsFloat['Canal Listón'] = (itemOtherMaterialsFloat['Canal Listón'] || 0) + canalListonFloat;


               // Canal Soporte (based on total valid area) - Keep existing logic, sum FLOAT
               ...
               itemOtherMaterialsFloat['Canal Soporte'] = (itemOtherMaterialsFloat['Canal Soporte'] || 0) + canalSoporteFloat;


               // Angular de Lámina (based on sum of full perimeters of segments MINUS deduction - using total valid perimeter sum) - Keep existing logic, sum FLOAT
               ...
               itemOtherMaterialsFloat['Angular de Lámina'] = (itemOtherMaterialsFloat['Angular de Lámina'] || 0) + angularLaminaFloat;


              // Patas (Soportes) - Calculation based on Canal Soporte quantity for THIS item (using total valid area for Canal Soporte calc) - Keep existing logic, sum FLOAT
              ...
              itemOtherMaterialsFloat['Patas'] = (itemOtherMaterialsFloat['Patas'] || 0) + patasFloat; // Add to float total


               // Canal Listón (para cuelgue) - Represents the number of 3.66m profiles needed to cut the hangers - Keep existing logic, sum FLOAT
               // Calculation based on rounded Patas quantity and Plenum for THIS item
               let roundedPatasForCuelgue = roundUpFinalUnit(patasFloat || 0); // Round patas float value calculated above
               if (plenum > 0 && roundedPatasForCuelgue > 0 && !isNaN(plenum)) {
                  ...
                  itemOtherMaterialsFloat['Canal Listón (para cuelgue)'] = (itemOtherMaterialsFloat['Canal Listón (para cuelgue)'] || 0) + ((roundedPatasForCuelgue * plenum) / 3.66);
               } else {
                   itemOtherMaterialsFloat['Canal Listón (para cuelgue)'] = (itemOtherMaterialsFloat['Canal Listón (para cuelgue)'] || 0) + 0;
               }


               // Acabado (Pasta, Cinta de Papel, Lija / Basecoat, Cinta malla) - Keep existing logic, sum FLOAT
               ...
               if (primaryPanelTypeForFinishing === 'Normal' || primaryPanelTypeForFinishing === 'Resistente a la Humedad' || primaryPanelTypeForFinishing === 'Resistente al Fuego' || primaryPanelTypeForFinishing === 'Alta Resistencia') {
                   ...
                   itemOtherMaterialsFloat['Pasta'] = (itemOtherMaterialsFloat['Pasta'] || 0) + (totalCieloAreaForPanelsFinishingCalc > 0 ? totalCieloAreaForPanelsFinishingCalc / 22 : 0);

                   // Cinta de Papel - Calculation based on Area
                   itemOtherMaterialsFloat['Cinta de Papel'] = (itemOtherMaterialsFloat['Cinta de Papel'] || 0) + (totalCieloAreaForPanelsFinishingCalc > 0 ? totalCieloAreaForPanelsFinishingCalc * 1 : 0);

                   // Lija Grano 120 - Calculation based on Area (estimated panels / 2)
                   itemOtherMaterialsFloat['Lija Grano 120'] = (itemOtherMaterialsFloat['Lija Grano 120'] || 0) + (totalCieloAreaForPanelsFinishingCalc > 0 ? (totalCieloAreaForPanelsFinishingCalc / PANEL_RENDIMIENTO_M2) / 2 : 0);

               } else if (primaryPanelTypeForFinishing === 'Exterior') { // Assuming 'Exterior' implies Durock or similar
                    // Basecoat (Durock) - Calculation based on Area
                   itemOtherMaterialsFloat['Basecoat'] = (itemOtherMaterialsFloat['Basecoat'] || 0) + (totalCieloAreaForPanelsFinishingCalc > 0 ? totalCieloAreaForPanelsFinishingCalc / 8 : 0);

                   // Cinta malla (Durock) - Calculation based on Area
                   itemOtherMaterialsFloat['Cinta malla'] = (itemOtherMaterialsFloat['Cinta malla'] || 0) + (totalCieloAreaForPanelsFinishingCalc > 0 ? totalCieloAreaForPanelsFinishingCalc * 1 : 0);
               }


               // --- Start New Fastener Calculation Logic (Cielo Item) - Uses FLOAT quantities ---

               // 1. Clavos con Roldana (Uso en Cielos)
               // Formula: Cantidad Total de Angulares (Cielos) x 5 + Cantidad Total de Soportes o Patas (Cielos) x 2
               // Usamos las cantidades float de Angular y Patas calculadas para este item.
               const angularLaminaFloatThisItem = itemOtherMaterialsFloat['Angular de Lámina'] || 0;
               const patasFloatThisItem = itemOtherMaterialsFloat['Patas'] || 0;

               itemOtherMaterialsFloat['Clavos con Roldana'] = (itemOtherMaterialsFloat['Clavos con Roldana'] || 0) + (angularLaminaFloatThisItem * 5) + (patasFloatThisItem * 2);


               // 3. Tornillos de 1" (Uso en Cielos - Fijación de Paneles)
               // Siempre se utiliza punta fina para paneles de cielo sobre estructura ligera.
               // Formula: Cantidad Total de Paneles (Cielos) x 40
               // Estimamos la cantidad FLOAT de paneles para este item basada en el área total válida
               const estimatedItemPanelsCieloFloat = totalCieloAreaForPanelsFinishingCalc > 0 ? totalCieloAreaForPanelsFinishingCalc / PANEL_RENDIMIENTO_M2 : 0;

               if (estimatedItemPanelsCieloFloat > 0) {
                    itemOtherMaterialsFloat['Tornillos de 1" punta fina'] = (itemOtherMaterialsFloat['Tornillos de 1" punta fina'] || 0) + (estimatedItemPanelsCieloFloat * 40);
                    // Tornillos 1" Punta Broca para cielos es 0 según la lógica
                    // itemOtherMaterialsFloat['Tornillos de 1" punta broca'] = (itemOtherMaterialsFloat['Tornillos de 1" punta broca'] || 0) + 0; // Ya está inicializado 0 si aplica
               }


               // 4. Tornillos de 1/2" (Uso en Cielos - Fijación de Canales Listón a Estructura Principal y Elementos de Soporte)
               // Formula: Cantidad Total de Canales Listón (Cielos) x 12 + Cantidad Total de Soportes o Patas (Cielos) x 2
                // Usamos las cantidades float de Canal Listón y Patas calculadas para este item.
               const canalListonFloatThisItem = itemOtherMaterialsFloat['Canal Listón'] || 0;
               const patasFloatForTornillosThisItem = itemOtherMaterialsFloat['Patas'] || 0;


               itemOtherMaterialsFloat['Tornillos de 1/2" punta fina'] = (itemOtherMaterialsFloat['Tornillos de 1/2" punta fina'] || 0) + (canalListonFloatThisItem * 12) + (patasFloatForTornillosThisItem * 2);
               // Tornillos 1/2" Punta Broca para cielos es 0 según la lógica
               // itemOtherMaterialsFloat['Tornillos de 1/2" punta broca'] = (itemOtherMaterialsFloat['Tornillos de 1/2" punta broca'] || 0) + 0; // Ya está inicializado 0 si aplica


               // Fulminantes (Cálculo GLOBAL, no por ítem. Inicializado a 0 si aplica) - Lógica movida al final de calculateMaterials

               // --- End New Fastener Calculation Logic (Cielo Item) ---


           } else if (type === 'cenefa') { // --- Cálculo para Cenefa ---
                const { cenefaLength, cenefaFaces, cenefaPanelType, metrajeLinear } = itemSpecs;

               itemMetraje.linear = metrajeLinear; // Use the calculated metrajeLinear from itemSpecs

               // Store panel details for accumulator update
               itemPanelDetails.cenefaPanelType = cenefaPanelType;
                // For cenefas, we need an estimated area for panel accumulator calculation.
                // Re-calculate the estimated area here using the validated length and faces.
                const assumedCenefaHeightForFinishing = 0.30; // Assumption!
                const estimatedCenefaAreaForPanels = (isNaN(cenefaLength) || cenefaLength <= 0 || isNaN(cenefaFaces) || cenefaFaces <= 0) ? 0 : (cenefaLength * cenefaFaces) * assumedCenefaHeightForFinishing;

                if (estimatedCenefaAreaForPanels > 0) {
                    itemPanelDetails.cenefaEstimatedArea = estimatedCenefaAreaForPanels;
                    itemPanelDetails.cenefaPanelType = cenefaPanelType; // Store type again for clarity
                } else {
                     itemPanelDetails.cenefaEstimatedArea = 0;
                }
               ...
               // --- Other Materials Calculation for THIS Cenefa Item (Structure, Finishing, Fasteners) ---
                ...
                // The length for Angular is the total length * number of faces (using actual validated dims).
                const totalCenefaLinearLength = (isNaN(cenefaLength) || cenefaLength <= 0 || isNaN(cenefaFaces) || cenefaFaces <= 0) ? 0 : cenefaLength * cenefaFaces;

                // Angular de Lámina (based on actual total length * faces) - Keep existing logic, sum FLOAT
                let angularLaminaFloat = totalCenefaLinearLength > 0 ? totalCenefaLinearLength / 2.44 : 0; // Divide by length of piece
                 itemOtherMaterialsFloat['Angular de Lámina'] = (itemOtherMaterialsFloat['Angular de Lámina'] || 0) + angularLaminaFloat;


                // Acabado (Pasta, Cinta de Papel, Lija / Basecoat, Cinta malla) - Based on the estimated *panel area* for this item.
                // Use the estimated area calculated for panel accumulator details.
                const estimatedCenefaAreaForFinishing = itemPanelDetails.cenefaEstimatedArea; // Use the value calculated above
                ...
                if (primaryPanelTypeForFinishing === 'Normal' || primaryPanelTypeForFinishing === 'Resistente a la Humedad' || primaryPanelTypeForFinishing === 'Resistente al Fuego' || primaryPanelTypeForFinishing === 'Alta Resistencia') {
                    ...
                    itemOtherMaterialsFloat['Pasta'] = (itemOtherMaterialsFloat['Pasta'] || 0) + (estimatedCenefaAreaForFinishing > 0 ? estimatedCenefaAreaForFinishing / 22 : 0);

                    // Cinta de Papel - Calculation based on Estimated Area
                    itemOtherMaterialsFloat['Cinta de Papel'] = (itemOtherMaterialsFloat['Cinta de Papel'] || 0) + (estimatedCenefaAreaForFinishing > 0 ? estimatedCenefaAreaForFinishing * 1 : 0);

                    // Lija Grano 120 - Calculation based on Estimated Area (estimated panels / 2)
                    itemOtherMaterialsFloat['Lija Grano 120'] = (itemOtherMaterialsFloat['Lija Grano 120'] || 0) + (estimatedCenefaAreaForFinishing > 0 ? (estimatedCenefaAreaForFinishing / PANEL_RENDIMIENTO_M2) / 2 : 0);

                    // Tornillos de 1" punta fina (for Yeso type panels on this item) - Keep existing logic, sum FLOAT
                     itemOtherMaterialsFloat['Tornillos de 1" punta fina'] = (itemOtherMaterialsFloat['Tornillos de 1" punta fina'] || 0) + (estimatedCenefaAreaForFinishing > 0 ? (estimatedCenefaAreaForFinishing / PANEL_RENDIMIENTO_M2) * 40 : 0); // 40 screws per panel (approx)


                } else if (primaryPanelTypeForFinishing === 'Exterior') { // Assuming 'Exterior' implies Durock or similar
                     // Basecoat (Durock) - Calculation based on Estimated Area
                    itemOtherMaterialsFloat['Basecoat'] = (itemOtherMaterialsFloat['Basecoat'] || 0) + (estimatedCenefaAreaForFinishing > 0 ? estimatedCenefaAreaForFinishing / 8 : 0);

                    // Cinta malla (Durock) - Calculation based on Estimated Area
                    itemOtherMaterialsFloat['Cinta malla'] = (itemOtherMaterialsFloat['Cinta malla'] || 0) + (estimatedCenefaAreaForFinishing > 0 ? estimatedCenefaAreaForPanels * 1 : 0); // Corregido typo 'estimatedCenefaAreaForFinishing' a 'estimatedCenefaAreaForPanels' para consistencia

                }

                // Tornillos para estructura (e.g., 1/2" to attach angular to ceiling/wall) - Keep existing logic, sum FLOAT
                ...
                let angularLaminaFloatThisItem = itemOtherMaterialsFloat['Angular de Lámina'] || 0; // Usamos el float calculado arriba
                // Assuming Cenefas attached to structure (wall/ceiling) would use broca screws.
                itemOtherMaterialsFloat['Tornillos de 1/2" punta broca'] = (itemOtherMaterialsFloat['Tornillos de 1/2" punta broca'] || 0) + (angularLaminaFloatThisItem * 5); // Use broca for attaching to structure
               ...
               itemOtherMaterialsFloat['Clavos con Roldana'] = 0;
               itemOtherMaterialsFloat['Tornillos de 1" punta fina'] = 0;
               itemOtherMaterialsFloat['Tornillos de 1" punta broca'] = 0;
               itemOtherMaterialsFloat['Tornillos de 1/2" punta fina'] = 0;
               itemOtherMaterialsFloat['Tornillos de 1/2" punta broca'] = 0;
               // Fulminantes are global.
           } // <<< ADDED missing closing brace for the else block

           // Return the calculated float quantities for other materials, panel details for accumulator, and metraje.
           return {
               otherMaterials: itemOtherMaterialsFloat,
               panelDetails: itemPanelDetails,
               itemMetraje: itemMetraje // Contains { area, linear }
           };
       };
    ...
    // Ahora se enfoca en la lectura de inputs, validación, llamada a la función de cálculo por ítem,
    // acumulación global (FLOAT para otros materiales), y manejo de resultados/errores/ajustes finales.
    const calculateMaterials = () => {
        ...
        // Panels are calculated and rounded per segment/area and summed globally here. This logic is retained.
        let panelAccumulators = {};
         PANEL_TYPES.forEach(type => {
            panelAccumulators[type] = {
                suma_fraccionaria_pequenas: 0.0, // Suma de cantidades FLOAT para áreas < threshold
                suma_redondeada_otros: 0 // Suma de cantidades REDONDEADAS para áreas >= threshold
            };
        });
        ...
        // Fijaciones y otros materiales (Pasta, Cinta, etc.) se acumulan como FLOAT aquí.
        ...
        itemBlocks.forEach(itemBlock => {
             ...
             try {
                ...
                console.log(`Procesando Ítem #${itemNumber} (ID: ${itemId}): Tipo=${type}`);
                 ...
                 // --- Metraje calculado para ESTE ítem (calcular aquí usando la lógica de calculateAndDisplayItemMetraje) ---
                 // Movemos la lógica de cálculo de metraje por ítem desde calculateAndDisplayItemMetraje a aquí para VALIDARLA
                 // y almacenarla en itemValidatedSpecs ANTES de pasarla a performSingleItemCalculations.
                  let itemMetrajeArea = 0; // Para Muros y Cielos
                  let itemMetrajeLinear = 0; // Para Cenefas
                  let hasValidSegmentForMaterials = false; // Flag to check if at least one segment is valid FOR MATERIAL calculation (used for Muro/Cielo)
                ...
                if (type === 'muro') {
                    ...
                    const segmentBlocks = itemBlock.querySelectorAll('.muro-segment');
                     ...
                     if (segmentBlocks.length === 0) {
                         ...
                         segmentBlocks.forEach((segBlock, index) => {
                             ...
                             // Calculate actual area and width for material calculation (only for valid segments)
                             let segmentActualArea = 0;
                             let segmentActualWidth = 0;
                             if (isSegmentValidForMaterials) {
                                  hasValidSegmentForMaterials = true; // Mark as valid if at least one segment is valid
                                  segmentActualArea = segmentWidth * segmentHeight;
                                  totalMuroAreaForPanelsFinishing += segmentActualArea; // Sum area for panels/finishing (using actual dims)
                                  segmentActualWidth = segmentWidth;
                                  totalMuroWidthForStructure += segmentActualWidth; // Sum width for structure (using actual dims)
                               }
                               ...
                               itemValidatedSpecs.segments.push({
                                   ...
                                   area: segmentActualArea, // Store actual area only if valid for materials
                         ...
                         // Store the calculated metraje area for the item
                         itemMetrajeArea = itemMuroMetrajeAreaCalc;
                         ...
                         itemValidatedSpecs.metrajeArea = itemMetrajeArea; // Store the calculated metraje area for the item
                    ...
                    const cieloPanelType = cieloPanelTypeSelect && !cieloPanelTypeSelect.closest('.hidden') ? cieloPanelTypeSelect.value : null;
                     ...
                     if (segmentBlocks.length === 0) {
                         ...
                         segmentBlocks.forEach((segBlock, index) => {
                              ...
                              // Calculate actual area and perimeter for material calculation (only for valid segments)
                              let segmentActualArea = 0;
                              let segmentActualPerimeter = 0;
                              if (isSegmentValidForMaterials) {
                                   hasValidSegmentForMaterials = true; // Mark as valid if at least one segment is valid
                                   segmentActualArea = segmentWidth * segmentLength;
                                   totalCieloAreaForPanelsFinishing += segmentActualArea; // Sum area (using actual dims)
                                   // --- CÁLCULO DEL PERÍMETRO COMPLETO POR SEGMENTO (usando actual dims) ---
                                   segmentActualPerimeter = 2 * (segmentWidth + segmentLength);
                                   totalCieloPerimeterForAngular += segmentActualPerimeter; // Sum of full perimeters for angular calculation
                                   // --- FIN CÁLCULO ---
                              }
                         ...
                         // Store the calculated metraje area for the item
                         itemMetrajeArea = itemCieloMetrajeAreaCalc;
                    ...
                    itemMetrajeLinear = itemCenefaMetrajeLinearCalc; // Store the calculated metraje linear
                    ...
                    itemValidatedSpecs.metrajeLinear = itemMetrajeLinear; // Store the calculated metraje linear
                ...
                if (itemSpecificErrors.length > 0) {
                     ...
                     // (Only push valid items to specs for reports that need detailed breakdown)
                     // We will only push valid items to currentCalculatedItemsSpecs later if material calculation was successful.
                     return; // Skip calculation and summing for this invalid item
                }

                 // --- Si el ítem es válido (no tiene itemSpecificErrors), PROCEDE AL CÁLCULO DETALLADO ---
                 // Ahora llamamos a la función dedicada para hacer la "matemática" del ítem
                 const calculatedItemResults = performSingleItemCalculations(itemValidatedSpecs);
                 console.log(`Ítem #${itemNumber}: Resultados de cálculo detallado (float):`, calculatedItemResults);


                 // --- Actualizar Acumuladores Globales usando los resultados de calculatedItemResults ---

                 // Acumular Paneles (usando los detalles de paneles retornados) - Lógica mantenida
                  const panelDetails = calculatedItemResults.panelDetails;
                  if (type === 'muro') {
                      panelDetails.muroSegmentsArea.forEach(segDetail => {
                          const panelesFloatFace1 = segDetail.segmentArea / PANEL_RENDIMIENTO_M2;
                          if (segDetail.segmentArea < SMALL_AREA_THRESHOLD_M2 && segDetail.segmentArea > 0) {
                              panelAccumulators[segDetail.panelTypeFace1].suma_fraccionaria_pequenas += panelesFloatFace1;
                          } else if (segDetail.segmentArea >= SMALL_AREA_THRESHOLD_M2) {
                              const panelesRoundedFace1 = roundUpFinalUnit(panelesFloatFace1); // Redondeo de la unidad del panel
                              panelAccumulators[segDetail.panelTypeFace1].suma_redondeada_otros += panelesRoundedFace1;
                          }

                          if (segDetail.panelTypeFace2) { // If there's a second face
                              const panelesFloatFace2 = segDetail.segmentArea / PANEL_RENDIMIENTO_M2;
                              if (segDetail.segmentArea < SMALL_AREA_THRESHOLD_M2 && segDetail.segmentArea > 0) {
                                  panelAccumulators[segDetail.panelTypeFace2].suma_fraccionaria_pequenas += panelesFloatFace2;
                              } else if (segDetail.segmentArea >= SMALL_AREA_THRESHOLD_M2) {
                                  const panelesRoundedFace2 = roundUpFinalUnit(panelesFloatFace2); // Redondeo de la unidad del panel
                                  panelAccumulators[segDetail.panelTypeFace2].suma_redondeada_otros += panelesRoundedFace2;
                              }
                          }
                      });
                  } else if (type === 'cielo') {
                       panelDetails.cieloSegmentsArea.forEach(segDetail => {
                           const panelesFloatCielo = segDetail.segmentArea / PANEL_RENDIMIENTO_M2;
                           if (segDetail.segmentArea < SMALL_AREA_THRESHOLD_M2 && segDetail.segmentArea > 0) {
                               panelAccumulators[segDetail.panelType].suma_fraccionaria_pequenas += panelesFloatCielo;
                           } else if (segDetail.segmentArea >= SMALL_AREA_THRESHOLD_M2) {
                                const panelesRoundedCielo = roundUpFinalUnit(panelesFloatCielo); // Redondeo de la unidad del panel
                                panelAccumulators[segDetail.panelType].suma_redondeada_otros += panelesRoundedCielo;
                           }
                       });
                  } else if (type === 'cenefa') { // Handle cenefa panel estimation for accumulator - Lógica mantenida
                      if (panelDetails.cenefaEstimatedArea > 0 && panelDetails.cenefaPanelType) {
                          const panelesFloatCenefa = panelDetails.cenefaEstimatedArea / PANEL_RENDIMIENTO_M2;
                          // For simplicity, let's treat cenefa panel area as a single area for accumulator logic.
                          // Assuming it behaves like "other" areas (rounded).
                          const panelesRoundedCenefa = roundUpFinalUnit(panelesFloatCenefa); // Redondeo de la unidad del panel
                           panelAccumulators[panelDetails.cenefaPanelType].suma_redondeada_otros += panelesRoundedCenefa;
                           console.log(`Cenefa #${itemNumber} (${panelDetails.cenefaPanelType}): Área estimada (${panelDetails.cenefaEstimatedArea.toFixed(2)}m2). Sumando redondeado (${panelesRoundedCenefa}) a acumulador.`);

                      }
                  }
                 // Fin Acumular Paneles
                ...
                for (const material in itemOtherMaterialsFloat) {
                    if (itemOtherMaterialsFloat.hasOwnProperty(material)) {
                        ...
                        if (!isNaN(floatQuantity)) { // Sum all valid numbers, even 0 if it results from calc
                             // --- Sumar la cantidad FLOAT directamente ---
                             otherMaterialsTotal[material] = (otherMaterialsTotal[material] || 0) + floatQuantity;
                             // --- Fin Sumar ---
                        }
                    }
                }
                 console.log(`Ítem #${itemNumber}: Otros materiales calculados (float) y sumados a total. Total parcial otros materiales (FLOAT):`, otherMaterialsTotal); // Actualizar log


                 // Acumular Metrajes Totales (usando el metraje calculado por ítem) - Lógica mantenida
                 const itemMetraje = calculatedItemResults.itemMetraje;
                 if (type === 'muro') {
                      totalMuroMetrajeAreaSum += itemMetraje.area;
                 } else if (type === 'cielo') {
                      totalCieloMetrajeAreaSum += itemMetraje.area;
                 } else if (type === 'cenefa') {
                     totalCenefaMetrajeLinearSum += itemMetraje.linear;
                 }
                ...
                // Push only if calculation was successful for this item (no errors before calling performSingleItemCalculations)
                currentCalculatedItemsSpecs.push(itemValidatedSpecs);
                 ...
                 // Continúa con el siguiente ítem si hay un error en este, pero no lo incluyas en los specs calculados ni en los totales.
             }
        ...
        // --- Combine Final Paneles with Other Materials Total (now float) ---
        // otros materiales total contiene ahora las cantidades FLOAT acumuladas
        let rawTotalMaterialsFloat = { ...finalPanelTotals, ...otherMaterialsTotal }; // Incluye Paneles (ya redondeados) y Otros (FLOAT)
        console.log("Total de materiales combinados (Paneles REDONDEADOS + Otros FLOAT):", rawTotalMaterialsFloat); // Actualizar log
        ...
        // Fulminantes = Total Clavos con Roldana (Proyecto) - Se calcula sobre el total FLOAT de clavos
        // Asegurarse de que Fulminantes se añada al total FLOAT antes de aplicar redondeo/merma
        rawTotalMaterialsFloat['Fulminantes'] = (rawTotalMaterialsFloat['Clavos con Roldana'] || 0); // Asignar el total FLOAT de clavos

        // --- Implement Fase 4: Redondeo, Merma, Empaque a los totales GLOBALES ---
        // Esto aplica a TODOS los materiales en rawTotalMaterialsFloat, EXCEPTO los paneles que ya se redondearon al final de su acumulación.
        // Vamos a aplicar merma y redondeo solo a los "Other Materials" (que ahora son FLOAT).
        // Los Paneles ya están redondeados y no se les aplicará merma ni un redondeo adicional aquí.

        const mermaPercentage = 0.05; // Define el porcentaje de merma (5% como ejemplo)
        let finalAdjustedMaterials = {};

        for (const material in rawTotalMaterialsFloat) {
            if (rawTotalMaterialsFloat.hasOwnProperty(material)) {
                const floatTotalQuantity = rawTotalMaterialsFloat[material];

                if (!isNaN(floatTotalQuantity) && floatTotalQuantity > 0) {
                    // Los Paneles ya fueron redondeados en su propia lógica de acumulación.
                    // No les aplicamos merma ni redondeo adicional en esta fase.
                    if (material.startsWith('Paneles de ')) {
                         finalAdjustedMaterials[material] = floatTotalQuantity; // Usar la cantidad ya redondeada
                         console.log(`Material: ${material}. Cantidad ya redondeada: ${floatTotalQuantity}. No se aplica merma ni redondeo adicional.`);
                    } else {
                         // Para Otros Materiales (Fijaciones, Pasta, Cinta, etc. - que están en floatTotalQuantity como FLOAT)
                         // 1. Redondeo hacia Arriba (primera vez para los floats)
                         let roundedQuantity1 = Math.ceil(floatTotalQuantity);
                         ...
                         // 3. Ajuste por Empaque/Unidades de Venta (Paso conceptual - requiere definir tamaños de empaque)
                         // Esta es una parte que necesitarías implementar basándote en los tamaños de caja de cada material.
                         // Por ahora, el resultado será la cantidad redondeada con merma.
                         let finalQuantityAdjustedForPackaging = roundedQuantity2; // Placeholder

                         finalAdjustedMaterials[material] = finalQuantityAdjustedForPackaging;
                         console.log(`Material: ${material}. Cantidad FLOAT: ${floatTotalQuantity}. Redondeo1: ${roundedQuantity1}. Con Merma: ${quantityWithMerma.toFixed(2)}. Redondeo2 (Final): ${roundedQuantity2}.`);
                    }

                } else {
                    // Si la cantidad es 0 o inválida, mantenerla así (o asegurar que sea 0).
                     finalAdjustedMaterials[material] = 0;
                }
            }
        }

        // Los totales finales para mostrar y descargar son ahora los 'finalAdjustedMaterials'
        ...
        // Si no hay errores y hay ítems calculados válidamente O metrajes totales positivos, muestra los resultados.
        // Verificamos si hay itemspecs válidos O si hay metrajes totales positivos.
        // O si hay materiales ajustados positivos (ya incluye paneles).
        if (currentCalculatedItemsSpecs.length > 0 ||
            ...
            Object.keys(finalAdjustedMaterials).some(material => finalAdjustedMaterials[material] > 0)) { // Añadir check de materiales ajustados positivos
            ...
            if (currentCalculatedItemsSpecs.length > 0) {
                ...
                resultsHtml += '<hr>';
                 ...
                 console.log("No hay ítems válidos calculados para mostrar en el detalle de ítems calculados.");
            ...
            // Tabla de Totales de Materiales
             // Solo añade la tabla si hay materiales ajustados con cantidad > 0
             if (Object.keys(finalAdjustedMaterials).some(material => finalAdjustedMaterials[material] > 0)) {
                 console.log("Generando tabla de materiales totales (AJUSTADOS)."); // Actualizar log
                 resultsHtml += '<h3>Totales de Materiales (Cantidades a Comprar):</h3>'; // Título actualizado

                 // finalAdjustedMaterials now holds the combined and adjusted totals
                 console.log("Total final de materiales calculados (combinados y ajustados):", finalAdjustedMaterials); // Actualizar log

                 // Sort materials alphabetically for consistent display
                 const sortedMaterials = Object.keys(finalAdjustedMaterials).sort(); // Iterar sobre las claves ajustadas

                resultsHtml += '<table><thead><tr><th>Material</th><th>Cantidad</th><th>Unidad</th></tr></thead><tbody>';

                sortedMaterials.forEach(material => {
                    const cantidad = finalAdjustedMaterials[material]; // <<< USAR finalAdjustedMaterials AQUÍ
                 ...
                 if (sortedMaterials.every(material => finalAdjustedMaterials[material] <= 0)) { // Check against adjusted amounts
                ...
                downloadOptionsDiv.classList.remove('hidden'); // Show download options
                  ...
                  console.log("No hay materiales ajustados con cantidad positiva para añadir tabla."); // Actualizar log
            ...
            console.log("Results HTML generated and added to DOM.");

            // --- Almacena el estado completo de los resultados, incluyendo Área de Trabajo y Metrajes Totales ---
            // Almacena los totales ajustados para la descarga
            lastCalculatedTotalMaterials = finalAdjustedMaterials; // <<< ALMACENAR LOS TOTALES AJUSTADOS
            ...
            lastErrorMessages = []; // Si llegamos aquí, no hay errores de validación que impiden mostrar resultados
            lastCalculatedWorkArea = currentCalculatedWorkArea; // Almacena el área de trabajo leído

             console.log("Estado de resultados (AJUSTADOS) almacenado para descarga."); // Actualizar log

        } else {
             console.log("No hay ítems válidos calculados ni metrajes totales positivos, ni materiales ajustados positivos."); // Actualizar log
    ...
    // Modificada para incluir los metrajes y usar los totales AJUSTADOS
    const generatePDF = () => {
        console.log("Iniciando generación de PDF...");
       // Ensure there are calculated results to download (materiales ajustados o metrajes)
       // Verificar si hay materiales ajustados POSITIVOS O metrajes totales positivos O ítems válidos calculados (para el detalle)
       if ((Object.keys(lastCalculatedTotalMaterials).length === 0 || Object.values(lastCalculatedTotalMaterials).every(cantidad => cantidad <= 0)) && // No hay materiales ajustados positivos
           (lastCalculatedTotalMetrajes['Muro Área Total Metraje (m²)'] <= 0 &&
            lastCalculatedTotalMetrajes['Cielo Área Total Metraje (m²)'] <= 0 &&
            lastCalculatedTotalMetrajes['Cenefa Lineal Total Metraje (m)'] <= 0) && // No hay metrajes positivos
           lastCalculatedItemsSpecs.length === 0) // No hay ítems válidos para el detalle
       {
           console.warn("No hay resultados calculados (materiales ajustados, metrajes o ítems válidos) para generar el PDF.");
       ...
       // --- Add Total Materials Table ---
        ...
        if (Object.keys(lastCalculatedTotalMaterials).some(material => lastCalculatedTotalMaterials[material] > 0)) {
            console.log("Añadiendo tabla de materiales totales (AJUSTADOS) al PDF."); // Actualizar log
             doc.setFontSize(14);
             doc.setTextColor(secondaryOliveRGB[0], secondaryOliveRGB[1], secondaryOliveRGB[2]);
             doc.setFont("helvetica", "bold");
            ...
            sortedMaterials.forEach(material => {
                const cantidad = lastCalculatedTotalMaterials[material]; // <<< USAR lastCalculatedTotalMaterials AQUÍ
            ...
            doc.save(`Calculo_Materiales_${new Date().toLocaleDateString('es-ES').replace(/\//g, '-')}.pdf`); // Filename with date
             ...
             console.log("No hay materiales ajustados con cantidad positiva para añadir tabla al PDF."); // Actualizar log
...
const generateExcel = () => {
    ...
    if ((Object.keys(lastCalculatedTotalMaterials).length === 0 || Object.values(lastCalculatedTotalMaterials).every(cantidad => cantidad <= 0)) && // No hay materiales ajustados positivos
        ...
        lastCalculatedItemsSpecs.length === 0) // No hay ítems válidos para el detalle
    {
         console.warn("No hay resultados calculados (materiales ajustados, metrajes o ítems válidos) para generar el Excel.");
   ...
   if (lastCalculatedItemsSpecs.length > 0) {
       ...
       sheetData.push([]); // Fila en blanco para espaciar
        ...
        console.log("No hay ítems válidos para mostrar en el detalle de ítems calculados en Excel."); // Actualizar log
        // Mostrar un mensaje si no hay ítems válidos calculados pero sí hay totales de metraje (ej: solo cenefas válidas)
        if (lastCalculatedTotalMetrajes['Muro Área Total Metraje (m²)'] > 0 ||
            lastCalculatedTotalMetrajes['Cielo Área Total Metraje (m²)'] > 0 ||
            lastCalculatedTotalMetrajes['Cenefa Lineal Total Metraje (m)'] > 0) {
   ...
   // Tabla de Totales de Materiales
    // Solo añade la tabla si hay materiales ajustados con cantidad > 0
    if (Object.keys(lastCalculatedTotalMaterials).some(material => lastCalculatedTotalMaterials[material] > 0)) {
        console.log("Generando tabla de materiales totales (AJUSTADOS) al Excel."); // Actualizar log
         ...
         console.log("No hay materiales ajustados con cantidad positiva para añadir tabla al Excel."); // Actualizar log
    }
   ...
   // Sin embargo, podemos ajustar anchos de columna estimados.
    ...
    if (col_widths[2]) col_widths[2] = Math.max(col_widths[2], 30); // Ajusta 30 si es necesario
     ...
     // Pero en el sheetData general, si hay detalle de ítems, la tabla de materiales empezaría después.
